    ...
    Pausable
{
    ...
    function mint(
        address user,
        uint256 tokenId
    ) external override onlyRole(PREDICATE_ROLE) whenNotPaused {
        _mint(user, tokenId);
    }
    ...
    function supportsInterface(
        ...
        view
        virtual
        override(AccessControl, ERC165Storage, ERC721URIStorage, IERC165)
        returns (bool)
    {
    ...
    function _beforeTokenTransfer(
        ...
        if (from != auctionAddress && from != address(0)) {
            require(
                block.timestamp > (startDate + 14 days),
                "NetVRKRootLand: Locked until January 25th"
            );
        }
    }
